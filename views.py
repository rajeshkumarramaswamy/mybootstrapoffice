...
def display(request):
	...
	query =	'select customerName, contactLastName, contactFirstName, employeeNumber, lastName, firstName, jobTitle, customerNumber, customerName from employees, customers where employeeNumber = salesRepEmployeeNumber and lastName = "%s" and firstName = "%s"' % (lastName, firstName)
	...
	for record in records:
		...
		query = 'select orderDate, status, productCode from orderdetails D, orders O where O.orderNumber=D.orderNumber and customerNumber = "%s" and orderDate between %s and %s' %(customerNumber,  from_date, to_date)
		cursor.execute(query)
		order_records = cursor.fetchall()
		for o_record in order_records:
			orderDate, status, productCode = o_record
			orderDate = str(orderDate)
			query = 'select productName from productlines L, products P where L.productLine = P.productLine and productCode = "%s" and productName=%s and P.productLine=%s' % (productCode, input_product, input_productline)
			cursor.execute(query)
			...
			for p_record in p_records:
				...
				'customerName': customerName, 'contactLastName': contactLastName, 'contactFirstName': contactFirstName, 'orderDate': orderDate, 'status': status, 
				}
